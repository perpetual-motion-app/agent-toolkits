...
async function main() {
    ...
    const toolkit = new index_1.PoolAnalyzerToolkit();
    try {
        ...
        const qualifiedPools = analyzedPools.filter((pool) => pool.analysis.meetsAllCriteria);
...
Analysis:
  ...
  - Quote Token Price: $${parseFloat(pool.attributes.quote_token_price_usd).toFixed(2)}
        ...
        analyzedPools
            .filter((pool) => !pool.analysis.meetsAllCriteria)
            .forEach((pool) => {
            pool.analysis.failedCriteria.forEach((criteria) => {
